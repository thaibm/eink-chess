...
- **Auto-save & Resume Game State (Lưu và Khôi phục ván đấu):**
  - Tự động lưu toàn bộ trạng thái ván cờ vào `localStorage` (`einkchess_saved_bot_game` cho Bot v1 và `einkchess_monolith_saved_game` cho Bot v2) ngay khi bắt đầu ván mới (`newGame()`) — ngay cả khi chưa đi nước cờ nào — cũng như sau mỗi nước đi của người chơi, nước đi của Bot, khi Đi lại (Undo), và sự kiện thoát trang (`beforeunload`/`pagehide`).
  - Dữ liệu lưu gồm: Cấp độ Bot (`botLevel`/`gRank`), bên cầm quân (`playerSide`/`gPC`), engine state/board (`fen`/`gB`, `history`/`gHist`, `turn`/`gTurn`, `castling`/`gCasB`, `epSquare`/`gEP`, `halfmoveClock`/`gHalf`), và nước đi gần nhất (`lastMove`/`gLast`).
  - **Tải lại trang (Reload / F5):** Trực tiếp khôi phục bàn cờ, thế cờ, bên cầm quân và lượt đi đang chơi dở mà không khởi động lại ván mới hay mở popup cài đặt. Khi người chơi bắt đầu ván mới cầm quân Trắng và chưa đi nước nào, việc tải lại trang vẫn bảo toàn nguyên vẹn bên cầm quân Trắng và cấu hình ván đấu đã chọn (không bị mất ván hay đảo sang quân Đen). Nếu đến lượt Bot đi dở, Bot sẽ tự động suy nghĩ và đi tiếp.
